...
def sign_up(request):
    ...
    if request.method == 'POST':
        ...
        if sign_up_a_form.is_valid() and sign_up_b_form.is_valid():
            ...
            x = phone_number_checker(phone_number)[0]
            if (phone_number_checker(phone_number)[0])==True:
            ...
            if existing_member_profile.count() != 0:
...
def summary(request):
    ...
    for provision in provisions_active:
        try:
            ...
            error_message = "Pending Provisions require the member they are provided to. Status reset to 'Available'"
    ...
    for need in needs_active:
        try:
            ...
            error_message = "Pending Needs require the member they are provided from. Status reset to 'Open'"
            need.status="Open"
            need.save()
...
def status_update(request,pk1,pk2,pk3,pk4):
    ...
    for provision in provisions_active:
        try:
            ...
            print("Pending Provisions require the member they are provided to. Status reset to 'Available'")
            provision.status="Available"
            provision.save()
    needs_active = Need.objects.filter(member_id=user_id).filter(Q(status='Pending')|Q(status='pending'))
    needs_activeLIST = []
    for need in needs_active:
        try:
            ...
            need.status="Open"
            print("Pending Needs require the member they are provided from. Status reset to 'Open'")
            need.save()
    action_items_open = Action_item.objects.filter(status="Open")
    context = {
    'user_is_authenticated':request.user.is_authenticated,
